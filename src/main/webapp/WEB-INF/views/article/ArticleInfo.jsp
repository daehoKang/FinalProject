	...
	<div class="article-main">
		<div class="article-header">
			...
			<h3 class="heading-large" style="border-bottom:2px solid #FD3A13; color:#FD3A13;
			 float: left; width:100%; padding: 0 0 6px; margin:0 0 18px;
			  text-align:center;">${ArticleInfo.ar_genre }</h3>
			...
			<div class="article-attribute">
				<span class="article-attribute-reportername">
					${ArticleInfo.ar_name} 기자 </span> <span class="article-attribute-date">등록일자
					: ${ArticleInfo.ar_date} </span> <span class="article-attribute-hits">
					조회수 : ${ArticleInfo.ar_hits} </span>
			</div>
		</div>
		<!-- 기사본문 -->
		<div class="article-main-text">
			<div class="article-main-img" style="text-align:center;">
				<!-- 기사 대표 사진 -->
				<img
					src="resources/img/${ArticleInfo.ar_img}"
					alt="사진이 없는 기사입니다.${ArticleInfo.ar_img}">
				<p class="##">
					<!-- 사진의 간략한 설명 -->
				</p>
			</div>
			<p class="##">${ArticleInfo.ar_detail} ${sesseionScope.loginMid}</p>
			<div class="article-recommend-outer">
				<!-- 기사 추천 -->
				<c:if test="${sesseionScope.loginMid != null}">
					<span class="article-recommend-inner"> <a
						href="articleRecommend?bool=good&ar_no=${ArticleInfo.ar_no}&mid=${sesseionScope.loginMid}&ar_genre=${ArticleInfo.ar_genre}"
						class="btn-recommend btn-good">${ArticleInfo.ar_thumbsup}<br>추천
					</a> <a
						href="articleRecommend?bool=bad&ar_no=${ArticleInfo.ar_no}&ar_genre=${ArticleInfo.ar_genre}"
						class="btn-recommend btn-bad">${ArticleInfo.ar_thumbsdown}<br>비추천
					</a>
					</span>
				</c:if>
			</div>
			...
			<div class="comments-write-board">
				<form action="commentsWrite" method="get">
					<div class="##">
						...
						<input type="hidden"
							name="cm_ar_no" value="${ArticleInfo.ar_no}"> <input
							type="hidden" name="ar_genre" value="${ArticleInfo.ar_genre}">
			...
			<c:forEach items="${commentsList}" var="comments">
				<div class="comments-list-wrapper">
					<div class="comments-list-top">
						<span class="comments-list-writter"> 아이디 : ${comments.cm_writer } </span> <span
							class="comments-date"> 날짜 : ${comments.cm_date } </span>
					</div>
					<div class="comment-list-bottom">
						...
						<c:if test="${comments.cm_writer == sesseionScope.loginMid}">
							<a
								href="commentsDelete?cm_no=${comments.cm_no}&ar_genre=${ArticleInfo.ar_genre}"
