...
div.modalContent { 
	position:fixed; top:20%; left:calc(50% - 350px); width:700px; height:350px; padding:20px 10px; background:#fff; border:2px solid #666; }
div.modalContent textarea { 
	font-size:16px; font-family:'맑은 고딕', verdana; padding:10px; width:680px; height:260px;
}
div.modalContent button {
	font-size:20px; padding:5px 10px; margin:5px 5px 5px 5px; background:#fff; border:1px solid #ccc; float:right;}
...
<body>
	...
	<div class="article-main">
		...
		<!-- 기사본문 -->
		<div class="article-main-text" style="text-align:center;">
			...
			<p class="##">${articleRead.ar_detail}</p>
			<div class="article-recommend-outer">
				<!-- 기사 추천 -->
				<c:if test="${sessionScope.loginMid != null}">
					<span class="article-recommend-inner"> <a
						href="articleRecommend?bool=good&ar_no=${articleRead.ar_no}&mid=${sessionScope.loginMid}&ar_genre=${articleRead.ar_genre}"
						class="btn-recommend btn-good">${articleRead.ar_thumbsup}<br>추천
					</a> <a
						href="articleRecommend?bool=bad&ar_no=${articleRead.ar_no}&mid=${sessionScope.loginMid}&ar_genre=${articleRead.ar_genre}"
			...
			<div class="comments-write-board">
				<form action="commentsWrite" method="get">
					<div class="##">
						...
						<input type="hidden" name="cm_writer" value="${sessionScope.loginMid}">
						<input type="hidden"
			...
			<c:forEach items="${commentsList}" var="comments">
				<div class="comments-list-wrapper">
					<div class="comments-list-top">
						<span class="comments-list-writter" style="font-weight:bold;"> 아이디 : ${comments.cm_writer } </span> <br><span
							class="comments-date"> ${comments.cm_date } </span>
						<c:if test="${comments.cm_writer == sessionScope.loginMid}">
				...
				<div class="replyModal">
					...
					<form action="commentsModify" method="get">
						<div>
							<textarea class="modal_repCon" name="cm_detail"></textarea>
						</div>
						<div>
								<input type="hidden" name="cm_no" value="${comments.cm_no}">
								<input type="hidden" name="ar_genre" value="${articleRead.ar_genre}">
